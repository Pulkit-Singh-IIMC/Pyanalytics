# -*- coding: utf-8 -*-
"""
Created on Sat Aug  8 20:05:09 2020
...
#dataset  -  understand  it  first  :  target  class  based  on  other  parameters #https://scikit-learn.org/stable/modules/generated/sklearn.datasets.load_wine.ht ml
...
cm=confusion_matrix(y_true=  y_test,  y_pred=y_predLG  ) #TN FP
#FN TP
#we  want  more  TN(Acutal-0,  Predict-0)  &  TP #FN  (Actual-1,  Predict-0)
cm
sns.heatmap(cm,  annot=True) 
plt.show();
#report
print(classification_report(y_test,  logModel.predict(X_test))) #predict
...
logit.fit().params #  fit  the  model
result  =  logit.fit() #Interpreting  the  results 
result.summary()
...
confusion_matrix(y_true=  y_test,  y_pred=y_predSM  ) #with  constant
logit2  =  sm.Logit(endog=y_train,  exog=sm.add_constant(X_train)) 
logit2.fit().params
#  fit  the  model result2  =  logit2.fit()
#Interpreting  the  results
 
result.summary2()
result.summary2()  #what  is  the  value  of  AIC  -  59.81  here
result.conf_int()
result.pred_table(threshold=.5) 
X_train.shape
X_test.shape #predict  test  data
result.predict(X_test)

#new  data
result.predict(newdata)  #.9  ~  1;  #.00  ~  0 
...
#Mention  2  applications  of  Logistic  Regression  (domain  and  what  can  we  predict) #end here
...
data.columns
#use  only  3  columns  for  constructing  a  model
data2  =  data[['Alcohol',  'Malicacid',  'Ash',  'target']] 
data2.head()
data2.describe()
...
#%%predict  target data2.sample()
newdata  =  pd.DataFrame({'Alcohol':[12,13],  'Malicacid':[72,100],  'Ash':[300, 1000]})
newdata

#X & y data2.head()
X  =  data2[['Alcohol',  'Malicacid',  'Ash']].values 
X
y  =  data2['target'].values 
y
data2.columns #train  and  test
